...
associating names with values. Note that while variable bindings can be read externally, they only
...
containing the assignment are soft scopes (loops, try-catch blocks, or struct blocks), the behavior
depends on whether the global variable `x` is defined:
...
is the hard scope rule again: since the assignment to `t` occurs inside of a function, which
...
extracting the bodies of the `greet`  and `sum_to′` into soft scope contexts. First, let's put the
...
in a file, it  prints an ambiguity warning and throws and undefined variable error. Let's see it
...
* global `s` is defined with the value `0` before when the loop is evaluated
...
when it appears on right hand side of `t = s + i`, even though that expression appears first and is
...
scope entirely as well as removing the potential for spooky action. We uncovered and fixed a significant number of bugs due to the removal of soft scope, vindicating the choice to get rid of it. of
bugs in the process. And there was much rejoicing! Well, no, not really. Because some people were
...
1. It's no longer convenient to copy and paste the code from inside a function body into the REPL
   to debug it—you have to add `global` annotations and the remove them again to go back;
...
a `global` annotation or accidentally shadows and existing global with a local in a soft scope,
